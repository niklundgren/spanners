...
contains
  ...
  subroutine eigenDM(omega,eigenvect,velocity)
    ...
    !write(*,*) "Info: symmetry imposed on velocities"
    !do ii=1,nptk
  ...
  subroutine phonon_espresso(kpoints,omegas,velocities,eigenvect)
    ...
    integer, parameter :: hdr_unit = 88
    integer, parameter :: bin_unit = 89
    integer :: n1, n2, n3, n4  ! adjust to your rank
    integer, dimension(4) :: shape
    integer, parameter :: U_TEXT = 20
    integer, parameter :: U_BIN  = 21
    integer, parameter :: U_INFO  = 22
    !-------------------------------------------------------
    ...
    PRINT *, "!! EPSILON T x ", eps(1, :) 
    PRINT *, "!! EPSILON T y ", eps(2, :) 
    PRINT *, "!! EPSILON T z ", eps(3, :) 
    do i=1,nat
       zeff(i,:,:)=transpose(born(:,:,i))
        PRINT *, "!! ZEFF T x ", i, zeff(i, 1, :) 
        PRINT *, "!! ZEFF T y ", i, zeff(i, 2, :) 
        PRINT *, "!! ZEFF T z ", i, zeff(i, 3, :) 
    end do
    read(1,*) qscell(1:3)
    ...
    PRINT *, "!! r z ", r(2, :) 
    PRINT *, "!! " 
    PRINT *, "!! CELL R x ", cell_r(1, :) 
    PRINT *, "!! CELL R y ", cell_r(2, :) 
    PRINT *, "!! CELL R z ", cell_r(3, :) 
    PRINT *, "!! " 
    PRINT *, "!! CELL G x ", cell_g(1, :) 
    PRINT *, "!! CELL G y ", cell_g(2, :) 
    PRINT *, "!! CELL G z ", cell_g(3, :) 
    PRINT *, "mm(1, :)", mm(1,:)
    PRINT *, "mm(2, :)", mm(2,:)
    PRINT *, "vol_r", volume_r
    
    ! ------------------------------------------------------------
    ! ===> Dump kpts to binary + header
    ! ------------------------------------------------------------
    ...
    open(unit=hdr_unit, file="kpoints_shape.txt", status="replace", action="write")
    write(hdr_unit, '(I10, I10)') n1, n2
    close(hdr_unit)
    ! 3) Write the raw data (stream unformatted for clean NumPy read)
    open(unit=bin_unit, file="kpoints.bin", access="stream", &
         form="unformatted", status="replace")
    write(bin_unit) kpoints
    close(bin_unit)
    ! 1) Grab the shape
    n1 = size(k,1)
    n2 = size(k,2)
    ! 2) Write the shape header (ASCII)
    open(unit=hdr_unit, file="k_shape.txt", status="replace", action="write")
    write(hdr_unit, '(I10, I10)') n1, n2
    close(hdr_unit)
    ! 3) Write the raw data (stream unformatted for clean NumPy read)
    open(unit=bin_unit, file="k.bin", access="stream", &
         form="unformatted", status="replace")
    write(bin_unit) k
    close(bin_unit)
             ...
             do m3=-ncell_g(3),ncell_g(3)
                ...
                do ik=1,nk
                   ...
                   if (geg.gt.0.0d0.and.geg/alpha/4.0d0.lt.gmax) then
                      ...
                      write(bin_unit, "(A10, I3, I3, I3, A8, 3F12.6)") &
                            ...
                            do ipol=1,3
                               ...
                               do jpol=1,3
                                  ...
                                  do i=1,3
                                     ...
                                     write(bin_unit, "(A10, I3, A5, I3, A5, I3, A5, I3)") &
                                     "     ~iat", iat, " jat ", jat, " ipl ", ipol, " jpl ", jpol
                                     write(bin_unit, "(A12, F12.6, A5, F12.6)") &
                                     "        1st ", zjg(jpol)*zeff(iat,i,ipol), " 2nd ", &
                                     zig(ipol)*zeff(jat,i,jpol)
                                     write(bin_unit, "(A12, F12.6, A5, F12.6)") &
                                     "        3rd ", zig(ipol)*zjg(jpol)*rr(iat,jat,i), &
                                     " 4th ", zig(ipol)*zjg(jpol)*(dgeg(i)/alpha/4.0+dgeg(i)/geg)
                                     write(bin_unit, "(A12, 2F12.6 )") &
                                     "          total- ", rddg
    ...
    ! 3) Write the raw data (stream unformatted for clean NumPy read)
    open(unit=bin_unit, file="ddyn_g.bin", access="stream", &
         form="unformatted", status="replace")
    write(bin_unit) ddyn_g
    close(bin_unit)
    ...
    n3 = size(eigenvect,3)
    ! 2) Write the shape header (ASCII)
    open(unit=hdr_unit, file="eigenvectors_shape.txt", status="replace", action="write")
    ...
    n2 = size(velocities,2)
    n3 = size(velocities,3)
    ! 2) Write the shape header (ASCII)
    open(unit=hdr_unit, file="velocities_shape.txt", status="replace", action="write")
    write(hdr_unit, '(I10, I10, I10)') n1, n2, n3
    close(hdr_unit)
    ! 3) Write the raw data (stream unformatted for clean NumPy read)
    open(unit=bin_unit, file="velocities.bin", access="stream", &
        form="unformatted", status="replace")
